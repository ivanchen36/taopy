...
class detail extends basecontroller
{
	...
	public function index(){
		...
		if($this->share){
			...
			$this->favorite_list = $ptx_favorite_sharing->search(array('share_id'=>$share_id),1,20,' user.user_id,user.avatar_remote,user.nickname, ptx_favorite_sharing.create_time as create_time ');
			$this->share_intro = parse_message($this->share['intro'],true);
			if($this->settings['ui_detail']['detail_may_like']){
				$round_shares = $ptx_share->get_round($this->share['category_id'], 8);
	...
	public function get_comments(){
		...
		$this->ajax_failed_response(T('get_comment_failed'));
	}
