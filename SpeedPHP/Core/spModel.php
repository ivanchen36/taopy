...
class spModel {
	...
	{
		$this->dbpre = $GLOBALS['G_SP']['db']['prefix'];
	...
	public function findAllJoin($conditions = null, $sort = null, $fields = null, $limit = null)
	{
		...
		if(is_array($this->linker)){
			foreach( $this->linker as $onelinker ){
				if($onelinker['enabled']){
					$table_name = $this->dbpre?$this->dbpre.$onelinker['fclass']:$onelinker['fclass'];
		...
		/*if($this->other_join&&$this->other_join['enabled']){
			$leftjoin_table[] = "LEFT JOIN {$this->dbpre}{$this->other_join['main_table']} {$this->other_join['main_alias']} ON {$this->other_join['main_alias']}.{$this->other_join['main_fkey']}={$this->dbpre}{$this->other_join['sec_table_alias']}.{$this->other_join['sec_mapkey']}";

			}*/
		if(count($leftjoin_table)>0){
			$leftjoin = join(" ",$leftjoin_table);
		}
		if(count($leftjoin_condition)>0){
			$where_array[] = '('.join(" AND ",$leftjoin_condition).')';
		}

		if(count($where_array)>0)$where = "WHERE ".join(" AND ",$where_array);
	...
	public function findCountJoin($conditions = null)
	{
		...
		$sql = "SELECT COUNT({$this->table}.{$this->pk}) AS SP_COUNTER FROM {$this->tbl_name} {$this->table} {$leftjoin} {$where}";
		$result = $this->_db->getArray($sql);
		return $result[0]['SP_COUNTER'];
	}
...
class spPager {
	...
	public function __call($func_name, $func_args){
		if( ( 'findAllJoin' == $func_name || 'findAll' == $func_name || 'findSql' == $func_name ) && 0 != $this->input_args[0]){
	...
	private function runpager($func_name, $func_args){
		...
		}elseif('findAllJoin'==$func_name){
			$total_count = $this->model_obj->findCountJoin($conditions);
		}else{
			$total_count = $this->model_obj->findCount($conditions);
		}
		//if($total_count > $pageSize){//old 作者目的是如果超出页码范围 显示最后一页 我们不希望这样。
		//if($total_count >= $pageSize){
		$total_page = ceil( $total_count / $pageSize );
		//$page = min(intval(max($page, 1)), $total_count); //old 作者目的是如果超出页码范围 显示最后一页 我们不希望这样。
		$page_for_pageData = min(intval(max($page, 1)), $total_count); // 对页码进行规范运算
		$this->pageData = array(
		//这个array里面的$page_for_pageData 原始都为$page，我把他们区分开了
				"total_count" => $total_count,                                 // 总记录数
				"page_size"   => $pageSize,                                    // 分页大小
				"total_page"  => $total_page,                                  // 总页数
				"first_page"  => 1,                                            // 第一页
				"prev_page"   => ( ( 1 == $page_for_pageData ) ? 1 : ($page_for_pageData - 1) ),         // 上一页
				"next_page"   => ( ( $page_for_pageData == $total_page ) ? $total_page : ($page_for_pageData + 1)),     // 下一页
				"last_page"   => $total_page,                                  // 最后一页
				"current_page"=> $page_for_pageData,                                        // 当前页
		...
		if('findSql'==$func_name){
			...
			return $this->model_obj->findAllJoin($conditions, $sort, $fields, $limit);
		}else{
